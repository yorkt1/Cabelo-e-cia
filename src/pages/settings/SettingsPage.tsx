...
export default function SettingsPage() {
  ...
  const { register, handleSubmit } = useForm({ defaultValues: { name: salon?.name, phone: salon?.phone, address: salon?.address } });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-border/60">
          <CardHeader><CardTitle className="font-display text-xl">Informações</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit((v) => { if (salon) { updateSalon(salon.id, v); toast.success("Salvo!"); } })} className="space-y-4">
              <div className="space-y-2"><Label>Nome do salão</Label><Input {...register("name")} /></div>
              <div className="space-y-2"><Label>Telefone</Label><Input {...register("phone")} /></div>
              <div className="space-y-2"><Label>Endereço</Label><Input {...register("address")} /></div>
              <Button type="submit" className="bg-primary text-primary-foreground hover:bg-primary/90">Salvar</Button>
            </form>
          </CardContent>
        </Card>
        <Card className="border-border/60">
          <CardHeader><CardTitle className="font-display text-xl">Horário de funcionamento</CardTitle></CardHeader>
