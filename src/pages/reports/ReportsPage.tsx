  ...
  const monthly = useMemo(() => {
    return Array.from({ length: 6 }).map((_, i) => {
      ...
      const v = transactions
        .filter((t) => t.type === "income" && dayjs(t.date).isSame(d, "month"))
  ...
  const byProf = useMemo(
    () =>
      professionals.map((p, i) => ({
        name: p.name.split(" ")[0],
        value: appointments.filter((a) => a.professionalId === p.id && a.status === "completed")
          .length,
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-border/60">
          <CardHeader>
            <CardTitle className="font-display text-xl">Faturamento mensal</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border/60">
          <CardHeader>
            <CardTitle className="font-display text-xl">Atendimentos por profissional</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
        ...
        <Card className="border-border/60 lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-display text-xl">Horários mais ocupados</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border/60 lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-display text-xl">Indicadores</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              ...
              <p className="font-display text-3xl font-semibold mt-2">{cancelRate}%</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground uppercase tracking-wider">
                Total de serviços
              </p>
              <p className="font-display text-3xl font-semibold mt-2">{services.length}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground uppercase tracking-wider">
                Profissionais ativos
              </p>
              <p className="font-display text-3xl font-semibold mt-2">{professionals.length}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground uppercase tracking-wider">
                Atendimentos totais
              </p>
              <p className="font-display text-3xl font-semibold mt-2">{appointments.length}</p>
            </div>
          </CardContent>
        </Card>
      </div>
